...
class FlowUpsamplerNet(nn.Module):
    ...
    def forward(self, gt=None, rrdbResults=None, z=None, epses=None, logdet=0., reverse=False, eps_std=None,
                y_onehot=None, test_input_size=None):

        if reverse:
            epses_copy = [eps for eps in epses] if isinstance(epses, list) else epses

            sr, logdet = self.decode(rrdbResults, z, eps_std, epses=epses_copy, logdet=logdet, y_onehot=y_onehot, test_input_size=test_input_size)
            return sr, logdet
        else:
            assert gt is not None
            assert rrdbResults is not None
            z, logdet = self.encode(gt, rrdbResults, logdet=logdet, epses=epses, y_onehot=y_onehot, test_input_size=test_input_size)

            return z, logdet

    def encode(self, gt, rrdbResults, logdet=0.0, epses=None, y_onehot=None, test_input_size=None):
        fl_fea = gt
        reverse = False
        level_conditionals = {}
        bypasses = {}
        ...
        for layer, shape in zip(self.layers, self.output_shapes):
            ...
            if(test_input_size is None):
                ...
                size = fl_fea.shape[2]
                level = int(np.log(test_input_size / size) / np.log(2))
    ...
    def decode(self, rrdbResults, z, eps_std=None, epses=None, logdet=0.0, y_onehot=None, test_input_size=None):
