...
class RRDBGlowNet(nn.Module):
    ...
    def normal_flow(self, gt, lr, y_onehot=None, epses=None, lr_enc=None, add_gt_noise=True, step=None):
        ...
        '''
        print('GT: {}\n'.format(gt.shape), flush=True)
        print('Z: {}\n'.format(z.shape), flush=True)
        print('LR: {}\n'.format(lr.shape), flush=True)
        print('LR_enc: {}\n\n'.format([tmp_enc.shape if type(tmp_enc) != type(None) else None for tmp_enc in lr_enc.values()]), flush=True)
        '''
        ...
        #return z, nll, logdet
        return z, nll, (-logdet) / float(np.log(2.) * pixels)
