...
class NonLocalSparseAttention(nn.Module):
    def __init__( self, n_hashes=4, channels=64, k_size=3, reduction=4, chunk_size=144, conv=nlsn_common.default_conv, res_scale=1):
        super(NonLocalSparseAttention,self).__init__()
        self.chunk_size = chunk_size
        self.n_hashes = n_hashes
        self.reduction = reduction
        self.res_scale = res_scale
        self.conv_match = nlsn_common.BasicBlock(conv, channels, channels//reduction, k_size, bn=False, act=None)
        self.conv_assembly = nlsn_common.BasicBlock(conv, channels, channels, 1, bn=False, act=None)
    ...
    def forward(self, input):
        ...
        x_embed_sorted = nlsn_common.batched_index_select(x_embed, mod_indices) #[N,n_hashes*H*W,C]
        y_embed_sorted = nlsn_common.batched_index_select(y_embed, mod_indices) #[N,n_hashes*H*W,C]
        ...
        ret = nlsn_common.batched_index_select(ret, undo_sort)#[N, n_hashes*H*W,C]
...
class NonLocalAttention(nn.Module):
    def __init__(self, channel=128, reduction=2, ksize=1, scale=3, stride=1, softmax_scale=10, average=True, res_scale=1,conv=nlsn_common.default_conv):
        super(NonLocalAttention, self).__init__()
        self.res_scale = res_scale
        self.conv_match1 = nlsn_common.BasicBlock(conv, channel, channel//reduction, 1, bn=False, act=nn.PReLU())
        self.conv_match2 = nlsn_common.BasicBlock(conv, channel, channel//reduction, 1, bn=False, act = nn.PReLU())
        self.conv_assembly = nlsn_common.BasicBlock(conv, channel, channel, 1,bn=False, act=nn.PReLU())
